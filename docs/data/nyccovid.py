...
with open('metro.csv', newline='') as csvfile:
    ...
    for row in reader:
        if row["date"] >= "2020-08-01":
            row = {'date': row['date'], 'stop_name': row['stop_name'], 'gtfs_longitude': row['gtfs_longitude'], 'gtfs_latitude': row['gtfs_latitude'], 'entries': row['entries'], 'exits': row['exits']}
            newcsv.append(row)

newcsv.sort(key=lambda x: x["date"]) # Sort the data by date for easier processing.

fieldnames = ['date', 'stop_name', 'gtfs_longitude', 'gtfs_latitude', 'entries', 'exits']  # declare the field names for the output csv.
