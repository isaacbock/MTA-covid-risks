...
function updateData() {
	...
	let allFileDates = [
		...
		// 210102, 210109, 210116, 210123, 210130,
		...
		// 211002,211009, 211016, 211023, 211030,
		// 211106, 211113, 211120, 211127,
		// 211204, 211211, 211218, 211225,
		220101
	];
	...
	function processDataHourly(data) {
		...
		data.forEach((reading) => {
			...
			reading.observedAt = moment.tz(
				date,
				"MM/DD/YYYY HH:mm:ss",
				"America/New_York"
			);
			reading.id =
				reading.UNIT + reading["C/A"] + reading.SCP + reading.observedAt;
			reading.unitID = reading.UNIT + reading["C/A"] + reading.SCP;

			let i = 0;
			let station = remoteComplexLookup.find(
				(d) => d.remote == reading.UNIT && d.booth == reading["C/A"]
			);
		...
		for (let i = 1; i < consolidatedData.length; i++) {
			...
			let entries =
				current.entries - previous.entries > 0
					? current.entries - previous.entries
					: 0;
			let exits =
				current.exits - previous.exits > 0 ? current.exits - previous.exits : 0;
			if (
				current.readingID != previous.readingID &&
				current.complex == previous.complex &&
		...
		for (let i = 0; i < aggregateTurnstileData.length; i++) {
			...
			if (
				aggregateTurnstileData[i].complex == currentStation.complex &&
				aggregateTurnstileData[i].date.isSame(currentStation.date, "day")
			) {
				total +=
					aggregateTurnstileData[i].entries + aggregateTurnstileData[i].exits;
	...
	function returnData() {
		...
		if (finalHourlyData.length != 0 && finalDailyData.length != 0) {
			...
			console.log(
				"(right click below, save as global variable, then run copy(temp1) to save data to clipboard)"
			);
			console.log(finalHourlyData);
			console.log("");
			console.log("");
			...
			console.log(
				"(right click below, save as global variable, then run copy(temp2) to save data to clipboard)"
			);
			console.log(finalDailyData);
		}
	}
